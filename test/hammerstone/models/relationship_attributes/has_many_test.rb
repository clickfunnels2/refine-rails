require "test_helper"
require 'support/filter_test_helper'

module Hammerstone::Refine::Conditions
  describe 'Has Many Test' do
    include FilterTestHelper

    around do |test|
      ApplicationRecord.connection.execute("CREATE TABLE notes (id bigint primary key, hmt_user_id bigint, invoice_id bigint);")
      ApplicationRecord.connection.execute("CREATE TABLE hmt_users (id bigint primary key, name varchar(255));")
      ApplicationRecord.connection.execute("CREATE TABLE invoices (id bigint primary key, hmt_user_id bigint, amount bigint);")
      test.call
      ApplicationRecord.connection.execute("DROP TABLE notes; DROP TABLE hmt_users; DROP TABLE invoices;")
    end

    it 'uses where in for has many users' do
      query = create_filter(single_builder)
      correct_sql = <<~SQL.squish
              SELECT "hmt_users".* FROM "hmt_users"
              WHERE ("hmt_users"."id" IN
              (SELECT "invoices"."hmt_user_id" FROM "invoices" WHERE ("invoices"."amount" = 1)))
              SQL
      assert_equal query.get_query.to_sql, correct_sql
    end

    it 'does not collapse two attributes on a has_many' do
      query = create_filter(two_attributes_no_collapse)
      correct_sql = <<~SQL.squish
              SELECT "hmt_users".* FROM "hmt_users"
              WHERE (("hmt_users"."id" IN
              (SELECT "invoices"."hmt_user_id" FROM "invoices"
              WHERE ("invoices"."amount" > 1))) AND
              ("hmt_users"."id" IN
              (SELECT "invoices"."hmt_user_id" FROM "invoices"
               WHERE ("invoices"."amount" < 10))))
              SQL
      assert_equal query.get_query.to_sql, correct_sql
    end

    it 'can handle deeply nested has many relationships' do
      query = create_filter(deeply_nested_blueprint)
      correct_sql = <<~SQL.squish
            SELECT "hmt_users".* FROM "hmt_users"
            WHERE ("hmt_users"."id" IN
            (SELECT "invoices"."hmt_user_id" FROM "invoices"
            WHERE "invoices"."id" IN
            (SELECT "notes"."invoice_id" FROM "notes"
            WHERE ("notes"."body" ILIKE '%foo%'))))
      SQL
      assert_equal query.get_query.to_sql, correct_sql
    end

    def deeply_nested_blueprint
      Hammerstone::Refine::Blueprints::Blueprint.new
      .criterion('invoice_note_body',
          clause: TextCondition::CLAUSE_CONTAINS,
          value: 'foo',
        )
    end

    def two_attributes_no_collapse
      Hammerstone::Refine::Blueprints::Blueprint.new
      .criterion('invoice_amount',
        clause: NumericCondition::CLAUSE_GREATER_THAN,
        value1: 1,
      )
      .and
      .criterion('invoice_amount',
        clause: NumericCondition::CLAUSE_LESS_THAN,
        value1: 10,
      )
    end

    def single_builder
      Hammerstone::Refine::Blueprints::Blueprint.new
      .criterion('invoice_amount',
        clause: NumericCondition::CLAUSE_EQUALS,
        value1: 1,
      )
    end

    def create_filter(blueprint)
      BlankTestFilter.new(blueprint,
      HmtUser.all,
      [
        TextCondition.new('name'),
        NumericCondition.new('invoice_amount').with_attribute('invoices.amount'),
        TextCondition.new('invoice_note_body').with_attribute('invoices.notes.body')
      ],
      HmtUser.arel_table)
    end
  end

  class HmtUser < ActiveRecord::Base
    has_many :invoices
    has_many :notes
  end

  class Invoice < ActiveRecord::Base
    has_many :notes
  end

  class Note < ActiveRecord::Base
  end
end

